...
off_t rand_get_file_size(const char* path, off_t original_size, struct fuse_file_info* fi_in,
    ...
    if (last_incomplete_block_size > 0) {
        ...
        last_block_real_size =
            rand_decode(aux_plain_buf, (unsigned char*)aux_cyphered_buf, last_incomplete_block_size, NULL);
    }
