...
//TODO add configuration to enable or disable this cache...
//TODO removed files should be evicted from cache...
//TODO and maybe last recently used entries...
ivdb fsize_cache;
...
#ifdef CACHE_ON
    //TODO: maybe this should be in an init with return value for catching errors
    init_hash(&fsize_cache);
#endif
...
int block_align_create (const char *path, mode_t mode, void *fi,struct fuse_operations nextlayer)
{
    ...
    ((struct fuse_file_info *) fi)->flags = ((((struct fuse_file_info *) fi)->flags & (~O_RDONLY)) & (~O_WRONLY)) | O_RDWR;
...
int process_read_block(char *buf, size_t size, uint64_t block_offset, uint64_t block_extra_offset, struct io_info* inf){
    ...
    if(size+block_extra_offset!=BLOCKSIZE){
...
#ifdef CACHE_ON
        value_db *value=NULL;
        hash_get(&fsize_cache, (char*) inf->path,&value);
        if(value!=NULL){
        ...
        bytes_to_read=((size+block_extra_offset)>=file_size-block_offset) ? size+block_extra_offset : file_size-block_offset;
...
int process_write_block(char *buf, size_t size, uint64_t block_offset, uint64_t block_extra_offset, struct io_info* inf){
    ...
    if(size==BLOCKSIZE && block_extra_offset==0){
        ...
        DEBUG_MSG("Remaining file bytes %llu Path %s, and block offset %llu, extra_offset %llu, size %zu\n",(unsigned long long int) remaining_file_bytes, inf->path,(unsigned long long int) block_offset, (unsigned long long int) block_extra_offset , size);
        ...
        if(block_offset>=file_size){
            ...
            if(block_extra_offset==0 && remaining_file_bytes<=size){
                ...
                bytes_to_write=size;
                write_buf=buf;
                DEBUG_MSG("Optimized Non-Append done file Path %s, and block offset %llu, extra_offset %llu, size %zu\n",inf->path,(unsigned long long int) block_offset, (unsigned long long int) block_extra_offset , size);
                ...
                //Since the bytes read can be appended with extra bytes we need to check the final amount of bytes to write
                bytes_to_write= (bytes_to_read>size+block_extra_offset) ? bytes_to_read : size+block_extra_offset;
    ...
    if(res<bytes_to_write){
        DEBUG_MSG("Error bytes_to_write > res file Path %s, and block offset %llu, extra_offset %llu, size %zu, res %d\n",inf->path,(unsigned long long int) block_offset, (unsigned long long int) block_extra_offset , size, res);
        return -1;
    }
    ...
    value->file_size=(file_size > block_offset+bytes_to_write) ? file_size : block_offset+bytes_to_write;
    hash_put(&fsize_cache, (char*) inf->path, value);
#endif
    return size;
}


int split_into_blocks(int io_type, char *buf, size_t size, uint64_t aligned_offset, uint64_t block_extra_offset, struct io_info* inf){
    ...
    while(buffer_bytes_processed<size){
        ...
        //if the value is higher than BLKSIZE, process the corresponding block (remove the extra_offset bytes that are not going to be read/written) and advance to the next block
        int size_to_process_in_block = (remaining_size_to_process+next_block_extra_offset>=BLOCKSIZE) ? BLOCKSIZE-next_block_extra_offset: remaining_size_to_process;

        DEBUG_MSG("Processing file Path %s, and block offset %llu, extra_offset %llu, size %zu, size_to_process %llu, buf pointer %zu\n",inf->path,(unsigned long long int) next_offset_to_process, (unsigned long long int) next_block_extra_offset, size, (unsigned long long int)remaining_size_to_process,buffer_bytes_processed);

        int res=0;
        if(io_type==WRITE){
            //process from buffer the next bytes by checking the number of bytes alredy processed (buffer_bytes_processed)
            res = process_write_block(&buf[buffer_bytes_processed], size_to_process_in_block, next_offset_to_process, next_block_extra_offset, inf); 
        } else{
            res = process_read_block(&buf[buffer_bytes_processed], size_to_process_in_block, next_offset_to_process, next_block_extra_offset, inf);   
        }
        if(res<0){
            DEBUG_MSG("Returning -1 offset %ld with blocksize %lu and res %d\n", next_offset_to_process, size_to_process_in_block, res);
            return res;
        }

        DEBUG_MSG("Processed file Path %s, and block offset %llu, extra_offset %llu, size %zu, size_to_process %llu, buf pointer %zu\n",inf->path,(unsigned long long int) next_offset_to_process, (unsigned long long int) next_block_extra_offset, size, (unsigned long long int)remaining_size_to_process,buffer_bytes_processed);
        ...
        //Only the first block processed may have an extra_offset i.e., not being read/written at the beggining of the block
    ...
    DEBUG_MSG("Returning from align offset %ld with size %lu size_processed %llu\n", next_offset_to_process, size, (unsigned long long int)buffer_bytes_processed); 
...
int block_align_read(const char *path, char *buf, size_t size, off_t offset,void *fi,struct fuse_operations nextlayer){
...
int block_align_write (const char *path, const char *buf, size_t size, off_t offset, void *fi, struct fuse_operations nextlayer){
...
int block_align_truncate (const char* path, off_t size,struct fuse_file_info *fi_in,struct fuse_operations nextlayer){
    ...
    //Size is smaller than current file size
        ...
        if(extra_bytes>0){
            DEBUG_MSG("read %s extra_bytes is %d offset is %llu\n",path, extra_bytes,(unsigned long long int)block_offset);
