 ...
 *
 	 ...
 	 Some cryptographic techniques requiring reading the whole block to decode it and require the whole block
 	 to cypher it.
...
static int
sfuse_read(const char *path, char *buf, size_t size, off_t offset,
...
static int
sfuse_write(const char *path, const char *buf, size_t size,
...
static int
sfuse_getattr(const char *path, struct stat *stbuf)
{
    int res;
...
static int
sfuse_fgetattr(const char *path, struct stat *stbuf,
                  struct fuse_file_info *fi)
{
    int res;
