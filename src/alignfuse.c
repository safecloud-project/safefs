/*
  ...
  Align fuse
  (c) 2016 2016 INESC TEC. Modified by J. Paulo (ADD other authrs here)
...
static int
alignfuse_read(const char *path, char *buf, size_t size, off_t offset,
...
static int
alignfuse_write(const char *path, const char *buf, size_t size,
...
static int
alignfuse_create(const char *path, mode_t mode, struct fuse_file_info *fi)
{
...
static int
alignfuse_open(const char *path, struct fuse_file_info *fi)
{
...
int 
init_align_driver(struct fuse_operations** fuse_operations, configuration config){
