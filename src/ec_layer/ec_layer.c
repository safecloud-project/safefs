...
static int
ec_getattr(const char *path, struct stat *stbuf)
{
    stbuf->st_uid = getuid();
    stbuf->st_gid = getgid();
...
static int
ec_fgetattr(const char *path, struct stat *stbuf,
...
static int
ec_opendir(const char *path, struct fuse_file_info *fi)
{
    int res;
...
static inline struct ec_dirp *
get_dirp(struct fuse_file_info *fi)
{
    return (struct ec_dirp *)(uintptr_t)fi->fh;
}
...
static int
ec_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
...
static int
ec_releasedir(const char *path, struct fuse_file_info *fi)
{
...
static int
ec_mknod(const char *path, mode_t mode, dev_t rdev)
{
    int res;
...
static int
ec_mkdir(const char *path, mode_t mode)
{
    int res;
...
static int
ec_rename(const char *from, const char *to)
{
    int res;
...
static int
ec_create(const char *path, mode_t mode, struct fuse_file_info *fi)
{
    char key[PATH_MAX + 1];
...
static int
ec_open(const char *path, struct fuse_file_info *fi)
{
    char key[PATH_MAX + 1];
    strncpy(key, path, strlen(path));
    if (!g_hash_table_contains(FILES, (gpointer) key)) {
...
static int
ec_read(const char *path, char *buf, size_t size, off_t offset,
              struct fuse_file_info *fi)
{
...
static int
ec_write(const char *path, const char *buf, size_t size,
               off_t offset, struct fuse_file_info *fi)
{
...
static int
ec_statfs(const char *path, struct statvfs *stbuf)
{
    int res;
...
static int
ec_release(const char *path, struct fuse_file_info *fi)
{
    char key[PATH_MAX + 1];
    strncpy(key, path, strlen(path));
...
int
init_ec_driver(struct fuse_operations** fuse_operations) {
...
int
clean_ec_driver(struct fuse_operations** fuse_operations) {
    erasure_destroy();
