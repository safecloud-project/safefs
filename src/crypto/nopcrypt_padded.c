...
void nop_padded_init(block_align_config config){
    NOP_BLOCKSIZE = config.block_size;
}
...
off_t nop_get_file_size_padded (const char* path, off_t original_size,struct fuse_file_info *fi, struct fuse_operations nextlayer){
...
int nop_get_cyphered_block_size_padded(int origin_size){
	...
	return origin_size+NOP_PADSIZE;
}
